...
export const ProveedorSchema = z.object({
  ...
  tipo_proveedor: z.enum(["Crédito", "Contado"], {
...
export const FacturaCompraSchema = z.object({
  ...
  numero_factura_proveedor: z
    .string()
    .min(1, "El número de factura del proveedor es requerido."),
  fecha_emision: z.date({
    required_error: "La fecha de emisión es requerida.",
  }),
  fecha_vencimiento: z
    .date({ invalid_type_error: "Fecha de vencimiento inválida." })
    .nullable(),
  tipo_pago: z.enum(["Contado", "Crédito"], {
